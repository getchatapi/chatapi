...
func (r *SQLiteBotRepository) Create(req *models.CreateBotRequest) (*models.Bot, error) {
	...
	_, err := r.db.Exec(`
		INSERT INTO bots (bot_id, name, created_at)
		VALUES (?, ?, ?)`,
		bot.BotID, bot.Name, bot.CreatedAt,
	)
	if err != nil {
...
func (r *SQLiteBotRepository) GetByID(botID string) (*models.Bot, error) {
	var bot models.Bot
	err := r.db.QueryRow(`
		SELECT bot_id, name, created_at
		FROM bots WHERE bot_id = ?`, botID,
	).Scan(&bot.BotID, &bot.Name, &bot.CreatedAt)
...
func (r *SQLiteBotRepository) List() ([]*models.Bot, error) {
	...
	for rows.Next() {
		var bot models.Bot
		if err := rows.Scan(&bot.BotID, &bot.Name, &bot.CreatedAt); err != nil {
